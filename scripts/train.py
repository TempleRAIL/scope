...
#
# This script trains a SO-SCOPE model
#------------------------------------------------------------------------------
...
# train function:
def train(model, scope_model, dataloader, dataset, device, optimizer, criterion, epoch, epochs):
    ...
    #for i, batch in enumerate(dataloader, 0):
        ...
        scope_prediction, kl_loss = scope_model(input_binary_maps)
        prediction = model(input_binary_maps)
        # calculate the total loss:
        ce_loss = criterion(prediction, scope_prediction.detach()).div(batch_size)
        # beta-vae:
...
# validate function:
def validate(model, scope_model, dataloader, dataset, device, criterion):
...
def main(argv):
    ...
    # load the weights
    #
    scope_mdl_path = './model/scope_model.pth'
    checkpoint = torch.load(scope_mdl_path, map_location=device)
    scope_model.load_state_dict(checkpoint['model'])

    # instantiate a model:
    model = so_scope(input_channels=NUM_INPUT_CHANNELS,
                  latent_dim=NUM_LATENT_DIM,
                  output_channels=NUM_OUTPUT_CHANNELS)
    # moves the model to device (cpu in our case so no change):
    model.to(device)
    ...
    for epoch in range(start_epoch+1, epochs):
        ...
        train_epoch_loss, train_kl_epoch_loss, train_ce_epoch_loss = train(
            model, scope_model, train_dataloader, train_dataset, device, optimizer, criterion, epoch, epochs
        )
        valid_epoch_loss, valid_kl_epoch_loss, valid_ce_epoch_loss = validate(
            model, scope_model, dev_dataloader, dev_dataset, device, criterion
        )
